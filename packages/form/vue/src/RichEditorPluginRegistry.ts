...
export type RichEditorTool = {
  label: string
  compactLabel?: string
  isActive?: (editor: Editor, field: FormComponent) => boolean
  canRun?: (editor: Editor, field: FormComponent) => boolean
  run: (editor: Editor, field: FormComponent) => void
}

export type RichEditorPlugin = {
  name: string
  extensions?: AnyExtension[] | ((field: FormComponent) => AnyExtension[])
...
export class RichEditorPluginRegistry {
  ...
  extensions(field: FormComponent): AnyExtension[] { return this.all().flatMap(plugin => typeof plugin.extensions === 'function' ? plugin.extensions(field) : plugin.extensions ?? []) }
  tool(name: string): RichEditorTool | undefined { for (const plugin of this.all()) if (plugin.tools?.[name]) return plugin.tools[name]; return undefined }
...
function assertPlugin(plugin: RichEditorPlugin, registered: Iterable<{ plugin: RichEditorPlugin }>): void {
  ...
  for (const [name, tool] of Object.entries(plugin.tools ?? {})) if (!tool.label.trim() || typeof tool.run !== 'function') throw new Error(`Rich editor tool [${name}] requires a label and run callback.`)
}
