...
function FieldShell({ field, path, error, children, onBlur, actionExecutor, classNames, icons, registries }: { field: FormField; path: string; error?: string; children: React.ReactNode; classNames?: FormClassNames; onBlur?: React.FocusEventHandler<HTMLDivElement>; actionExecutor?: ActionExecutor; icons?: Record<string, SchemaIconRenderer>; registries?: FormRendererRegistries }) {
  ...
  return (
    <div {...extra.attributes} className={`min-w-0 ${field.columnSpanFull ? 'col-span-full' : `${responsivePlacementClasses} ${responsiveFullSpanClasses(field.columnSpan)}`} ${classNames?.field ?? ''} ${extra.className}`.trim()} data-computed={field.computed ? 'true' : undefined} data-field={field.name} data-slot="field" onBlur={onBlur} style={field.columnSpanFull ? undefined : placementStyles(field)}>
      ...
      <div className={`${field.inlineLabel ? 'mt-2 sm:col-start-2 sm:mt-0' : 'mt-2'} ${classNames?.controlWrapper ?? ''}`.trim()} data-slot="control-wrapper">
...
export function FieldRenderer(props: FieldRendererProps) {
  ...
  switch (field.type) {
    ...
    case 'color-picker': control = (field.format ?? 'hex') === 'hex'
      ? <input {...common} className="size-12 rounded-(--inlay-radius) border-0 bg-(--inlay-surface) p-1 shadow-xs ring-1 ring-(--inlay-border) focus-visible:ring-2 focus-visible:ring-(--inlay-accent)" onChange={(e) => change(path, e.target.value)} type="color" value={String(value ?? '#000000')} />
    ...
    case 'time-picker': {
      ...
      control = <input {...common} className={inputClass} max={field.max ?? undefined} min={field.min ?? undefined} onChange={(e) => change(path, e.target.value)} step={field.seconds ? 1 : undefined} type={date && time ? 'datetime-local' : date ? 'date' : 'time'} value={String(value ?? '')} />
      break
    }
...
function AffixActions({ actions = [], runtime }: { actions?: ActionResource[]; runtime: ReturnType<typeof useActionRuntime> }) {
  return <>{actions.map(action => <ActionButton action={action} aria-label={action.label} className={`${buttonExtraSmallClass} px-2 py-1 text-xs`} key={action.instanceKey ?? action.name} runtime={runtime}>{action.icon ? <span aria-hidden="true" data-icon={action.icon}>{action.icon}</span> : action.label}</ActionButton>)}</>
}
...
function ToggleButtons({ extraAttributes, extraClassName, field, path, value, update }: { extraAttributes?: Record<string, string | number | boolean | null>; extraClassName?: string; field: FormField; path: string; value: unknown; update: FieldRendererProps['update'] }) {
  ...
  return <div aria-required={field.required || undefined} className="flex flex-wrap gap-2" role="group">{field.options?.map((option) => <button {...safeAttributes} aria-pressed={selected.includes(String(option.value))} className={`${secondaryButtonClass} aria-pressed:border-(--inlay-accent) aria-pressed:bg-(--inlay-accent) aria-pressed:text-(--inlay-accent-foreground) ${extraClassName ?? ''}`.trim()} disabled={field.disabled} key={option.value} onClick={() => update(path, field.multiple ? selected.includes(String(option.value)) ? selected.filter((item) => item !== String(option.value)) : [...selected, String(option.value)] : option.value)} type="button">{option.label}</button>)}</div>
}
...
function BuilderBlocks({ field, path, value, update, childUpdate, liveChange, defaultLive, values, errors, renderers, registries, actionExecutor, uploadProgress, wizardStepValidator }: { field: FormField; path: string; value: unknown; update: FieldRendererProps['update']; childUpdate: FieldRendererProps['update']; liveChange: FieldRendererProps['liveChange']; defaultLive?: LiveConfig | null; values: Record<string, unknown>; errors: FormErrors; renderers?: SchemaRendererRegistry; registries?: FormRendererRegistries; actionExecutor?: ActionExecutor; uploadProgress?: number | null; wizardStepValidator?: WizardStepValidator }) {
  ...
  return <div aria-labelledby={`inlay-form-${path.replaceAll('.', '-')}-label`} className="grid gap-4" data-slot="builder" role="group">
    {rows.map(({ item, index, key }) => {
      ...
      return <fieldset className="rounded-(--inlay-radius) border border-(--inlay-border) bg-(--inlay-surface) p-4 shadow-xs" data-block={item.type} data-has-errors={nestedErrorCount(errors, `${path}.${index}`) ? 'true' : undefined} data-slot="builder-item" key={key}>
