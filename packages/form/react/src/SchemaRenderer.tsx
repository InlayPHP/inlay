...
function LayoutRenderer({ component, path, ...context }: ComponentRendererProps) {
  ...
  if (component.type === 'actions') return <SchemaActionsRenderer actions={component.actions ?? []} alignment={component.alignment} executor={context.actionExecutor} icons={context.icons} />
  ...
  const baseClass = component.type === 'fieldset' && component.contained !== false ? 'rounded-(--inlay-radius-lg) border border-(--inlay-border) bg-(--inlay-surface) p-(--inlay-space-card) shadow-xs' : ''
  return (
...
function SchemaActionsRenderer({ actions, alignment = 'start', className, executor, icons, slot = 'schema-actions' }: { actions: ActionResource[]; alignment?: 'start' | 'center' | 'end' | 'between'; className?: string; executor?: ActionExecutor; icons?: Record<string, SchemaIconRenderer>; slot?: string }) {
  ...
  return <div className={`flex flex-wrap gap-2 ${justify} ${className ?? ''}`.trim()} data-slot={slot}>{actions.map(action => <ActionButton action={action} icons={icons} key={action.instanceKey ?? action.name} runtime={runtime} />)}<ActionDialog runtime={runtime}>{dialogRuntime => <ActionForm runtime={dialogRuntime} />}</ActionDialog></div>
}
...
function ExtraActionSlot({ actions, alignment, legacyAlignment, executor, icons, slot }: { actions?: ActionResource[]; alignment?: 'start' | 'center' | 'end' | 'between'; legacyAlignment?: 'start' | 'center' | 'end' | 'between'; executor?: ActionExecutor; icons?: Record<string, SchemaIconRenderer>; slot: 'header' | 'footer' }) {
  ...
  return <SchemaActionsRenderer actions={actions} alignment={justify} className={slot === 'footer' ? 'mt-5 border-t border-(--inlay-border) pt-4' : ''} executor={executor} icons={icons} slot={`${slot}-actions`} />
}
...
function SectionRenderer({ component, path, ...context }: ComponentRendererProps) {
  ...
  const padding = component.compact ? 'p-3' : 'p-(--inlay-space-card)'
  ...
  return <section {...extra.attributes} className={`${context.classNames?.section ?? ''} rounded-(--inlay-radius-lg) border border-(--inlay-border) ${component.secondary ? 'bg-(--inlay-surface-muted)' : 'bg-(--inlay-surface) shadow-xs'} ${padding} ${component.aside ? 'md:grid md:grid-cols-[minmax(0,16rem)_1fr] md:gap-6' : ''} ${extra.className}`.trim()} data-secondary={component.secondary ? 'true' : 'false'} data-slot="section"><header className="flex items-start justify-between gap-4">{heading}<div className="flex flex-wrap items-center justify-end gap-2"><ExtraActionSlot actions={component.headerActions} alignment={component.headerActionsAlignment} executor={context.actionExecutor} icons={context.icons} slot="header" />{component.collapsible ? <button aria-controls={contentId} aria-expanded={!collapsed} className="rounded-(--inlay-radius) px-2 py-1 text-sm font-medium text-(--inlay-muted) hover:bg-(--inlay-surface-muted) hover:text-(--inlay-text) focus-visible:outline-2 focus-visible:outline-(--inlay-focus-ring-color)" onClick={toggle} type="button">{collapsed ? 'Expand' : 'Collapse'}</button> : null}</div></header>{!collapsed ? <div className={component.aside ? '' : 'mt-5'} id={contentId}><SchemaSlot context={context} path={path} schema={component.headerSchema} slot="header" /><SchemaRenderer columnScope="layout" columns={component.columns ?? 1} dense={component.dense} gap={component.gap} path={path} schema={component.schema ?? []} {...context} /><SchemaSlot context={context} path={path} schema={component.footerSchema} slot="footer" /><ExtraActionSlot actions={component.footerActions} alignment={component.footerActionsAlignment} legacyAlignment={component.footerAlignment} executor={context.actionExecutor} icons={context.icons} slot="footer" /></div> : null}</section>
}
  ...
  if (isHtml) {
    return (
      <div {...extra.attributes} className={`${component.copyable ? 'inline-flex items-start gap-2' : ''} ${className}`.trim()} data-content-type="html" data-slot="text" title={component.tooltip ?? undefined}>
        ...
        {component.copyable ? <button aria-label={`Copy ${component.label}`} className="shrink-0 cursor-copy rounded-md border border-(--inlay-border) bg-transparent px-2 py-1 text-xs font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--inlay-focus-ring-color)" onClick={() => void copy()} title={copyStatus || 'Copy'} type="button">Copy</button> : null}
...
function WizardRenderer({ component, path, ...context }: ComponentRendererProps) {
  ...
  return <section aria-busy={validatingStep} className={`${context.classNames?.wizard ?? ''} rounded-(--inlay-radius-lg) border border-(--inlay-border) bg-(--inlay-surface) p-(--inlay-space-card) shadow-xs`.trim()} data-slot="wizard">
    <ExtraActionSlot actions={component.headerActions} alignment={component.headerActionsAlignment} executor={context.actionExecutor} icons={context.icons} slot="header" />
    <ol className="flex gap-2 overflow-x-auto pb-1" role="list">{steps.map((item, index) => { const complete = index < active; const icon = complete ? item.completedIcon ?? item.icon : item.icon; const numberBadge = `flex size-7 items-center justify-center rounded-full text-xs font-semibold ${complete || active === index ? 'bg-(--inlay-accent) text-(--inlay-accent-foreground)' : 'bg-(--inlay-surface-strong) text-(--inlay-muted-strong)'}`; return <li className="min-w-0" key={item.name}><button aria-current={active === index ? 'step' : undefined} data-has-errors={hasNestedErrors(context.errors, [item], path) ? 'true' : undefined} className="flex min-h-11 items-center gap-2 whitespace-nowrap rounded-(--inlay-radius-sm) border px-3 py-2 text-left text-base text-(--inlay-muted-strong) transition hover:border-(--inlay-border-strong) hover:bg-(--inlay-surface-subtle) enabled:hover:text-(--inlay-fg-strong) focus-visible:outline-2 focus-visible:outline-(--inlay-focus-ring-color) aria-current:border-(--inlay-accent-border) aria-current:bg-(--inlay-accent-soft) aria-current:text-(--inlay-accent-strong) aria-current:font-semibold disabled:cursor-not-allowed disabled:opacity-50 sm:text-sm" disabled={!component.skippable && index > active} onClick={() => selectStep(index)} type="button"><span aria-hidden="true" className={numberBadge}>{icon ? <NamedIcon context={context} name={icon} /> : index + 1}</span><span><span className="block font-medium">{item.label}</span>{item.description ? <span className="block text-xs font-normal text-(--inlay-muted)">{item.description}</span> : null}</span></button></li> })}</ol>
    ...
    <ExtraActionSlot actions={component.footerActions} alignment={component.footerActionsAlignment} legacyAlignment={component.footerAlignment} executor={context.actionExecutor} icons={context.icons} slot="footer" />
  </section>
}

function wizardControlClass(action: ActionResource | null | undefined, primary: boolean): string {
  const base = 'inline-flex items-center gap-2 rounded-(--inlay-radius) px-3 py-2 text-sm font-semibold shadow-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--inlay-focus-ring-color) disabled:cursor-not-allowed disabled:opacity-50'
  ...
  return primary ? `${base} bg-(--inlay-accent) text-(--inlay-accent-foreground) hover:border-(--inlay-accent-strong) hover:bg-(--inlay-accent-strong)` : `${base} border border-(--inlay-border) bg-(--inlay-surface) text-(--inlay-text) hover:bg-(--inlay-surface-muted)`
}
