...
export function RichEditorControl({ common, field, inputAttributes = {}, onChange, value }: RichEditorControlProps) {
  ...
  return <div className={`overflow-hidden rounded-(--inlay-radius) bg-(--inlay-surface) shadow-xs ring-1 ${common['aria-invalid'] ? 'ring-(--inlay-danger)' : 'ring-(--inlay-border)'} focus-within:ring-(length:--inlay-focus-ring-width) focus-within:ring-(--inlay-focus-ring-color)`} data-content-mode={field.contentMode ?? 'html'} data-slot="rich-editor">
    ...
    {mentionQuery ? <div aria-label={`${mentionQuery.provider.trigger} mention suggestions`} className="border-b border-(--inlay-border) bg-(--inlay-surface) p-2 shadow-sm" data-slot="mention-suggestions" role="listbox">
      {mentionLoading ? <p className="px-2 py-1.5 text-sm text-(--inlay-muted)" role="status">Searching mentions…</p> : mentionOptions.length ? mentionOptions.map(option => <button aria-selected="false" className="flex w-full items-center rounded-md px-2 py-1.5 text-left text-sm text-(--inlay-text) hover:bg-(--inlay-surface-muted) focus-visible:outline-2 focus-visible:outline-(--inlay-focus-ring-color)" key={option.id} onMouseDown={event => event.preventDefault()} onClick={() => insertMention(option)} role="option" type="button"><span className="mr-2 font-mono text-(--inlay-accent)">{mentionQuery.provider.trigger}</span>{option.label}</button>) : <p className="px-2 py-1.5 text-sm text-(--inlay-muted)">No mentions found.</p>}
    </div> : null}
