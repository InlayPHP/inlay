...
<template>
  <section :aria-label="title" aria-modal="true" :class="`flex max-h-[min(90dvh,60rem)] min-w-0 w-full max-w-7xl flex-col overflow-hidden rounded-(--inlay-radius) bg-(--inlay-surface-muted) shadow-2xl ring-1 ring-(--inlay-border) ${props.class ?? ''}`" data-contract="inlay.media-manager.v1" role="dialog" :style="themeStyle">
