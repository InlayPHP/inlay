...
import type { InfolistIconRenderer, InfolistRendererRegistries } from './types'
...
const renderer = computed(() => props.icons?.[props.name]
  ?? props.icons?.['*']
  ?? (props.registries?.icon ? toRaw(props.registries.icon).get(props.name) : undefined)
  ?? (props.registries?.icon ? toRaw(props.registries.icon).get('*') : undefined))
