...
describe('Vue Table', () => {
  ...
  it('emits complete dark semantic tokens for a themed table', () => { const data = resource([column({})]); const theme = { contract: 'inlay.themes.v1' as const, name: 'test', tokens: { surface: '#ffffff', foreground: '#111827', 'surface-subtle': '#f8fafc' }, darkTokens: { surface: '#18181b', foreground: '#fafafa', 'surface-subtle': '#27272a' } }; const view = render(Table, { props: { resource: data, theme } }); const darkStyle = document.head.querySelector('[data-slot="table-dark-theme"]'); expect(darkStyle?.textContent).toContain('--inlay-surface: #18181b !important'); expect(darkStyle?.textContent).toContain('--inlay-foreground: #fafafa !important'); expect(darkStyle?.textContent).toContain('--inlay-surface-subtle: #27272a !important'); view.unmount() })
